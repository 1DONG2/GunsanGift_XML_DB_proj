...
ini_set("memory_limit","2345M");
    			...
    			foreach($arr as $row){
					...
					if($row->IsThisSpecial->IsThisHoleIntheWall=="O"){ //이쪽을 바꿔야함
    ...
    $data = "";
    $data .= "<table>";
    $data .= "<th>순번</th><th>업소명</th><th>업종</th><th>도로명주소</th><th>행정동</th><th>전화번호</th><th>군산사랑상품권</th><th>백년가게</th><th>맛집</th><th>착한가격업소</th><th>모범음식점</th>";
    if (file_exists('../realfinalresult.xml')) {
        ...
        foreach($arr as $row){
            if($row->IsThisSpecial->IsThisHoleIntheWall=="O"){ //이쪽을 바꿔야함
                ...
                $data .= "<td>" . $row->IsThisSpecial->IsThisMillenniumStore . "</td>";
                $data .= "<td>" . $row->IsThisSpecial->IsThisHoleIntheWall . "</td>";
                $data .= "<td>" . $row->IsThisSpecial->IsThisChackanStore . "</td>";
                $data .= "<td>" . $row->IsThisSpecial->IsThisGoodRestaurant . "</td>";
                $data .= "</tr>";
            }
